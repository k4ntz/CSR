...
def rewardShaping(action, already_done, results, ep_length, state):
    results = list(results)
    next_state, reward, done = results[0], results[1], results[2]
    if reward > 0:
        reward = reward * 100
        already_done = True
    elif already_done:
        ...
        reward = -500
    elif np.array_equal(state['image'], next_state['image']):
        reward = -2
        ...
        reward = -1
    results[1] = reward
    return tuple(results), already_done
...
def simulate(
    ...
    while (steps and step < steps) or (episodes and episode < episodes):
        ...
        if manager is not None:
            action, agent_state = agent(obs, done, agent_state, manager=manager)
        else:
        ...
        for i in range(len(ep_length)):
            ep_length[i] += 1
        if shaped_reward and not is_eval:
            for env_id in range(len(envs)):
                try:
                    results[env_id], terminated[env_id] = rewardShaping(action[env_id], terminated[env_id], results[env_id], ep_length[env_id], prev_obs[env_id])
                except:
                    results[env_id], terminated[env_id] = rewardShaping(action[env_id], terminated[env_id], results[env_id], ep_length, prev_obs[env_id])
