...
import q_learning.utils4dqn as utils
...
def train(config):
    ...
    for i in range(config.num_episodes):
        if total_steps > config.steps:
            break
        state = env.reset()
        ep_reward = 0
        ep_length = 0
        terminated = False
        if config.test_while_train and (eval_check == 0 or eval_check > config.eval_every):
            print("Start evaluation.")
            test(config, dqn, 100, tr_writer, total_steps)
            eval_check = 0
            # dqn.Q_function.train()
        while True:
        ...
        eval_check += ep_length
        print("finished ep: {} , total_step {}, ep_rew {} len {}. esp {}. time_passed {}, memory_size {} ".format(
            i,
            total_steps,
            ep_reward,
            ep_length,
        ...
        tr_writer.add_scalar("ep_length", ep_length, global_step=total_steps)
        ...
        tr_writer.add_scalar("ep_reward", ep_reward_norm, global_step=total_steps)
        tr_writer.add_scalar("Avg. reward per step", avg_reward_norm, global_step=total_steps)
...
def test(config, dqn = None, levels=5, tr_writer=None, eval_steps=None):
    """Test routine"""
    ...
    for i in range(levels):
        ...
        while True:
            ...
            action = dqn.choose_action(state, is_test=True)
    ...
    if eval_steps is not None:
        assert tr_writer is not None
        tr_writer.add_scalar("eval_reward", total_reward / levels, global_step=eval_steps)
    env.close()
    if dqn is not None:
        agent.train()
    if success >= (levels / 2):
